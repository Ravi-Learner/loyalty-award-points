...
public interface IAwardProgramActivityRepository extends JpaRepository<AwardProgramActivity, Long> {
	...
	@Query(value = "select customer_id as customerId, month(create_date) as month,year(create_date) year, round(sum(points_earned),2) as points"
			+ " from  award_program_activity where customer_id = ?1 group by customer_id, year(current_date), month(create_date) order by customer_id, year, month asc", nativeQuery = true)
	...
	@Query(value = "select customer_id as customerId, month(create_date) as month,year(create_date) year, round(sum(points_earned),2) as points"
			+ " from  award_program_activity group by customer_id, year(current_date), month(create_date) order by customer_id, year, month asc", nativeQuery = true)
